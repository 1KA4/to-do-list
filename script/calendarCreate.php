...
?>
let currentMonth = <?php echo($select_date[1] - 1); ?>;
let currentYear = <?php echo $select_date[0]; ?>;
let years = [];
...
$(".next").on("click", function (e) {
    ...
    if (currentMonth > 11) {
        ...
        currentMonth = 0;
        if (currentYear + 1 <= maxYear) { currentYear += 1; } setYear(currentYear);
    }
...
$(".prev").on("click", function (e) {
    e.stopPropagation(); let minYear = Math.min(...years); currentMonth--; if (currentMonth < 0) {
        currentMonth = 11;
...
function monthFillDays(iYear, iMonth) {
    ...
    if (iMonth < "10") { iMonth = "0" + (iMonth + 1); } else { iMonth++; } for (let i = 1; i <= days; i++) {
        let day = 0; if (i < 10) {
            day = "0" + i;
        } else { day = i; } $("#monthDays").append('<div class="col col-md" name="' + iYear + '-' + iMonth + '-' + day + '">' +
...
function showTask() {
    ...
    for (let i = 0; i < db_array.length; i++) {
        if (db_array[i] == date) {
            ...
            $("#viewTaskModal .modal-body").append(`<div class="task mb-4 align-items-center">
                <p class="mb-0"><strong>${tasknum}. </strong> ${db_array[index]}</p>
                <a href="../php/taskmanager.php?taskId=${db_array[index-3]}" class="btn btn-outline-danger d-flex align-items-center" name="taskDeleteBtn">
